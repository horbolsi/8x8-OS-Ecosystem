...
const checks = [
  ...
  ['live people truth is zero', js.includes("fact('LIVE PEOPLE', '0')")],
  ['transmission truth is none', js.includes("fact('TRANSMISSION', 'NONE')")],
  ...
  ['no precise geolocation API', !/navigator\.geolocation|getCurrentPosition\(|watchPosition\(/.test(js)],
  ['no device capture API', !/getUserMedia\(|navigator\.(bluetooth|serial|usb)|requestDevice\(/.test(js)],
  ['no wallet or payment API', !/window\.(ethereum|solana)|new PaymentRequest|transaction\.sign/i.test(js)],
